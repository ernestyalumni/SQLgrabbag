...
# redo the session as such:
# session.close()
# session = Session()
## so the engine is ok.  session is faulty.  
...
## I wanted to look up a company by its description, and all I had to do was match a part of its name
qChev = session.query(security).filter(security.Name.match('Chevron'))
qChev.scalar()
